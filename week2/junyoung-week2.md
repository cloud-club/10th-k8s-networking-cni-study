...
IPAM을 따로 표시한 것은 기능을 구분하기 위해서다. 실제 할당 처리는 별도 IPAM 플러그인 호출이나 노드 에이전트와의 통신 등 구현에 따라 달라질 수 있다.

핵심은 **kubelet이 직접 IP를 골라주는 것이 아니라, 런타임이 호출한 네트워크 구현의 IPAM 기능이 할당을 처리한다**는 점이다. 현재의 일반적인 CRI 구성에서 CNI를 직접 관리·호출하는 주체는 런타임이다. 과거 kubelet의 CNI 관리 옵션은 Kubernetes 1.24에서 제거되었다.
